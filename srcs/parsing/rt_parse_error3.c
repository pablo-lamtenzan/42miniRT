...
/*   Updated: 2020/07/08 14:19:48 by plamtenz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool			rt_parse_error_square_fill(t_obj *obj, char **values,
		t_square s)
{
...
bool				rt_parse_error_square(t_obj *obj, char *line)
{
	...
	if (!frmt(&values[1], 10)
			|| !parse_obj_bonus_properties(obj, &values[11], SQUARE))
	...
	if ((s.color.b = (double)ft_atoi(values[10])) < 0x0 || s.color.b > 0xFF)
		return (ft_error_free(OBJ_PARAMS, values));
	return (rt_parse_error_square_fill(obj, values, s));
}
...
bool				rt_parse_error_triangle(t_obj *obj, char *line)
{
	...
	if (!(values = split_multicharset(line, " ,", &len)) || len < 13)
		return (values ? ft_error_free(OBJ_PARAMS, values) : ft_error(4));
	if (!frmt(&values[1], 12)
			|| !parse_obj_bonus_properties(obj, &values[13], TRIANGLE))
		return (ft_error_free(OBJ_PARAMS, values));
...
bool				rt_parse_error_pyramid(t_scene *s, t_obj *obj, char *line)
{
	...
	if (!(values = split_multicharset(line, " ,", &len)) || len < 12)
		return (values ? ft_error_free(OBJ_PARAMS, values) : ft_error(4));
	if (!frmt(&values[1], 11)
			|| !parse_obj_bonus_properties(obj, &values[12], PYRAMID))
		return (ft_error_free(OBJ_PARAMS, values));
