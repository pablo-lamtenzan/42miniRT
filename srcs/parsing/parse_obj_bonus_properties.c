...
bool            parse_damier(t_obj *obj, char **values)
{
	...
	if ((obj->color_damier.g = (double)ft_atoi(values[2])) < 0x0 || obj->color_damier.r > 0xff)
		return (false);
	if ((obj->color_damier.b = (double)ft_atoi(values[3])) < 0x0 || obj->color_damier.r > 0xff)
        return (false);
...
bool            parse_textures(t_obj *obj, char **values)
{
    ...
    while (((values[1][i] >= 'a' && values[1][i] <= 'z') ||
			values[1][i] == '/' || (values[1][i] >= '0' && values[1][i] <= '9')) && values[1][i] != '.')
        i++;
    if (!(values[1][i + 4] && values[1][i] == '.' && values[1][i + 1] == 'x' && values[1][i + 2] == 'p' &&
            values[1][i + 3] == 'm' && values[1][i + 4] == '"' && !values[1][i + 5]) || values[2])
        return (false);
    if (i + 3 > 256)
		return (false);
    if ((map = malloc(sizeof(t_uvmap))))
	{
    	...
    	if (!(map->addr = mlx_xpm_file_to_image(obj->mlx_ptr, s, &map->x, &map->y))
        	    || !(map->img = (t_color *)mlx_get_data_addr(map->addr, &aux, &aux, &aux)))
        	return (false);
    	// nb of thread = 1 ? 
    	obj->uvmap = map;
